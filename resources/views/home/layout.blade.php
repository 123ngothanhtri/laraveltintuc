...
<body>
    <header x-data="{ mobileMenuOpen : false }" class="flex flex-wrap flex-row justify-between md:items-center md:space-x-4 bg-green-300  py-6 px-6 relative">
        ...
        <nav class="absolute md:relative top-16 left-0 md:top-0 z-20 md:flex flex-col md:flex-row md:space-x-6 font-semibold w-full md:w-auto bg-green-100 shadow-md rounded-lg md:rounded-none md:shadow-none md:bg-transparent p-6 pt-0 md:p-0"
            :class="{ 'flex' : mobileMenuOpen , 'hidden' : !mobileMenuOpen}" @click.away="mobileMenuOpen = false">
            <a href="{{ url('/') }}" class="block py-1 text-gray-600 ">Trang chủ</a>
            <a href="{{ url('/lien-he') }}" class="block py-1 text-gray-600 ">Liên hệ</a>
        </nav>
    </header>
